...
class BaiTapRenderDanhSachPhim extends Component {
    ...
    renderMangPhim = () => {
        ...
        const arrPhim = mangPhim.map((phim, index) => {
            return <div className="col-3 pb-2" key={index}>
                <div className="card">
                    <img className={`card-img-top ${style.hinhAnhPhim}`} src={phim.hinhAnh} alt="" />
                    <div className="card-body">
                        <h4 className="card-title">{phim.tenPhim}</h4>
        <p className="card-text">{phim.moTa.length > 100 ? <span>{phim.moTa.substr(0, 100)} ...</span> : <span>{phim.moTa}</span>}</p>
                    </div>
...
export default BaiTapRenderDanhSachPhim;